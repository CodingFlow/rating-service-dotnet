...
using VerifyCS = RequestDecoratorGenerator.UnitTests.CSharpSourceGeneratorVerifier<RequestDecoratorGenerator.Main>;

namespace RequestDecoratorGenerator.UnitTests;

public class Tests
{
    private Assembly implementationAssembly;

    // Ensure TestLibrary shows up in referenced assemblies
    private GetRatingsQuery query;

    [SetUp]
    public void Setup()
    {
        implementationAssembly = GetAssembly("RequestDecoratorGenerator");
    }

    [Test]
    public async Task SimpleGetPost()
    {
        var source = await ReadCSharpFileByName(true, "RequestDecorator");
        var generated = await ReadCSharpFileByName(true, "RequestDecoratorExtensions");
        ...
        await new VerifyCS.Test
        {
            ...
            TestState = {
                ...
                GeneratedSources =
                {
                    (typeof(Main), "RequestDecoratorExtensions.generated.cs", SourceText.From(generated, Encoding.UTF8, SourceHashAlgorithm.Sha256)),
                },
            },
        }.RunAsync();
    }

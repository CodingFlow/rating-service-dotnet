...
RUN dotnet pack -r linux-x64 -c release -o ./local-nuget-feed ./Service.Application.Common/
RUN dotnet pack -r linux-x64 -c release -o ./local-nuget-feed ./Service.Api.Common/
...
RUN dotnet publish -r linux-x64 -c release -o /app --no-restore

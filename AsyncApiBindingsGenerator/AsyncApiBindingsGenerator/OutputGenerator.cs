    ...
    internal static class OutputGenerator
    {
        public static (string source, string className) GenerateSpecOutputs(AsyncApiDocument spec, string assemblyName)
        {
            ...
            var formattedCases = channels.Select(channel =>
            {
                ...
                var restMethodWithPathPart = $"{ToPascalCase(restMethod)}{ToPascalCase(pathPart)}";
                var mergeMethodName = $"merge{restMethodWithPathPart}";
                ...
                var jsonTypeInfo = $"{restMethodWithPathPart}{requestType}Context.Default.{restMethodWithPathPart}{requestType}";


                return $@"            case [{pathPartsFormatted}]:
                await restHandler.{methodName}(client, requestData, pathParts, {variableName}, {mergeMethodNameWithComma}{jsonTypeInfo}, cancellationToken);
                break;";
            });
...
using {serviceNamespacePart}.Api.Queries;
using {serviceNamespacePart}.Api.Commands;
...
";
        ...
        private static string CreateWithBlock(AsyncApiMessage messageReference)
        {
            ...
            if (IsPayloadPropertyExists(messageReference, "queryParameters"))
            {
                ...
                ? {queryParameterKey}.Split("",""){parseMethod}.ToList()

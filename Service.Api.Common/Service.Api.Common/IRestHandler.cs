...
public interface IRestHandler
{
    ...
    Task HandleDelete<TRequest, TResponse>(NatsClient client, Request<JsonNode> requestData, string[] pathParts, IDeleteHandler<TRequest, TResponse> getHandler, Func<TRequest, Dictionary<string, string>, string[], TRequest> requestMerger, CancellationToken cancellationToken);
    Task HandleDelete<TRequest>(NatsClient client, Request<JsonNode> requestData, string[] pathParts, IDeleteHandler<TRequest> getHandler, Func<TRequest, Dictionary<string, string>, string[], TRequest> requestMerger, CancellationToken cancellationToken);
    Task HandleDelete(NatsClient client, Request<JsonNode> requestData, string[] pathParts, IDeleteHandler getHandler, CancellationToken cancellationToken);
}